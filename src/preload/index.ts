...
const api = {
  ...
  kanban: {
    ...
    onTaskProgress: (callback: (data: { taskId: string; progress: string; message: string; items?: Array<{ label: string; status: 'pending' | 'in_progress' | 'completed' }>; activity?: { type: string; label: string; detail?: string }; subagents?: Array<{ name: string; status: string }> }) => void) => {
      const listener = (_event: Electron.IpcRendererEvent, payload: { taskId: string; progress: string; message: string; items?: Array<{ label: string; status: 'pending' | 'in_progress' | 'completed' }>; activity?: { type: string; label: string; detail?: string }; subagents?: Array<{ name: string; status: string }> }) =>
        callback(payload)
