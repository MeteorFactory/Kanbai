...
const api = {
  ...
  terminal: {
    ...
    onCompanionCreate: (callback: (data: { provider: string; workspaceId?: string; makeTarget?: string; projectPath?: string; projectName?: string }) => void) => {
      const listener = (
        _event: Electron.IpcRendererEvent,
        payload: { provider: string; workspaceId?: string; makeTarget?: string; projectPath?: string; projectName?: string },
      ) => callback(payload)
  ...
  git: {
    ...
    worktreeMergeAndCleanup: (repoPath: string, worktreePath: string, worktreeBranch: string, ticketLabel: string, targetBranch?: string) =>
      ipcRenderer.invoke(IPC_CHANNELS.GIT_WORKTREE_MERGE_AND_CLEANUP, { repoPath, worktreePath, worktreeBranch, ticketLabel, targetBranch }),
  ...
  kanban: {
    ...
    getWorkingTicket: (workspaceId: string): Promise<{ ticketNumber: number | null; isCtoTicket: boolean; type?: string; status?: string } | null> =>
    ...
    linkConversation: (cwd: string, taskId: string, workspaceId: string, aiProvider?: string): Promise<string | null> =>
      ipcRenderer.invoke(IPC_CHANNELS.KANBAN_LINK_CONVERSATION, { cwd, taskId, workspaceId, aiProvider }),
    prequalify: (data: { title: string; description: string; priority?: string; type?: string; targetProjectId?: string; isCtoTicket?: boolean; hasAttachments?: boolean; hasComments?: boolean }): Promise<{ suggestedType: string; suggestedPriority: string; clarifiedDescription: string; isVague: boolean; splitSuggestions?: Array<{ title: string; description: string; type: string; priority: string }> } | PrequalifyError | null> =>
    ...
    onTaskProgress: (callback: (data: { taskId: string; progress: string; message: string; phase?: string; items?: Array<{ label: string; status: 'pending' | 'in_progress' | 'completed' }>; activity?: { type: string; label: string; detail?: string }; subagents?: Array<{ name: string; status: string }> }) => void) => {
      const listener = (_event: Electron.IpcRendererEvent, payload: { taskId: string; progress: string; message: string; phase?: string; items?: Array<{ label: string; status: 'pending' | 'in_progress' | 'completed' }>; activity?: { type: string; label: string; detail?: string }; subagents?: Array<{ name: string; status: string }> }) =>
        callback(payload)
  ...
  updates: {
    ...
    resolveCommand: (tool: string): Promise<{ command: string } | null> =>
      ipcRenderer.invoke(IPC_CHANNELS.UPDATE_RESOLVE_COMMAND, { tool }),
    listInstalled: () =>
      ipcRenderer.invoke(IPC_CHANNELS.UPDATE_LIST_INSTALLED),
  ...
  // Skills Store
  skillsStore: {
    fetch: (force?: boolean): Promise<{ repos: SkillStoreRepo[]; skills: SkillStoreEntry[] }> =>
      ipcRenderer.invoke(IPC_CHANNELS.SKILLS_STORE_FETCH, { force }),
    install: (projectPath: string, skill: SkillStoreEntry): Promise<{ success: boolean; error?: string }> =>
      ipcRenderer.invoke(IPC_CHANNELS.SKILLS_STORE_INSTALL, { projectPath, skill }),
  },
