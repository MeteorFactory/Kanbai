    ...
    if (fs.existsSync(dataPath)) {
      ...
      if (s) {
        return {
          ...
          aiMemoryRefactorInterval: defaults.aiMemoryRefactorInterval,
...
# - DONE    -> auto-commit worktree (merge is handled by the AI via prompt instructions)
...
switch ($TicketStatus) {
  "DONE" {
    Auto-CommitWorktree
    # Merge is now handled by the AI via prompt instructions (like ticket status update).
...
export function registerKanbanHandlers(ipcMain: IpcMain): void {
  ...
  ipcMain.handle(
    ...
    async (_event, { id, workspaceId }: { id: string; workspaceId: string }) => {
      ...
      // Soft-delete: archive the task instead of removing it
      taskToDelete.archived = true
      taskToDelete.updatedAt = Date.now()
