  ...
  useEffect(() => {
    const unsubscribe = window.kanbai.kanban.onTaskProgress((data) => {
      useKanbanStore.setState((s) => {
        if (!data.progress && !data.message && !data.phase && (!data.activity || data.activity.type === 'idle')) {
        ...
        return {
          agentProgress: {
            ...s.agentProgress,
            [data.taskId]: { progress: data.progress, message: data.message, phase: data.phase, items: data.items, activity: data.activity, subagents: data.subagents },
          },
        }
      })
    })
